...
class SERVUSTVVODIE(InfoExtractor):
    IE_NAME = 'servustv:vod'
    _VALID_URL = r'https?://(?:www\.)?servustv\.tv/p/(?P<uploader_id>.+?)\+(?P<id>[^/?#&]+)'
    _TESTS = [{
        'url': 'https://servustv.tv/p/pdp+3mTzOl4WR',
        'info_dict': {
            'id': '3mTzOl4WR',
            'ext': 'mp4',
        ...
        'url': 'https://servustv.tv/p/pdpreplay+D-RD-xSZg',
        'only_matching': True,
    }]
    ...
    def _real_extract(self, url):
        ...
        broadcast = self._download_json(
            'https://graphigo.prd.servustv.tv/', vod_id,
  ...
  pastBroadcast(permlink:"%s+%s") {
    ...
    viewCount
        ...
        formats = self._extract_m3u8_formats(
            broadcast['playbackUrl'], vod_id, 'mp4', 'm3u8_native')[1]
...
class SERVUSTVStreamIE(InfoExtractor):
    IE_NAME = 'servustv:stream'
    _VALID_URL = r'https?://(?:www\.)?servustv\.com/\w+/\w+/\w./(?P<id>[\w.*/])'
    ...
    def _real_extract(self, url):
        ...
        user = self._download_json(
            'https://graphigo.prd.servustv.tv/', display_name,
            data=json.dumps({'query': '''query {
  userByDisplayName(displayname:"%s") {
    ...
    username
        ...
        formats = self._extract_m3u8_formats(
            'https://live.prd.servustv.tv/hls/live/%s.m3u8' % username,
            display_name, 'mp4')
        #    https://rbmn-live.akamaized.net/hls/live/2023676/geodachallow210820WsbkFp1Pri/master.m3u8
        self._sort_formats(formats)
        return {
            'id': display_name,
