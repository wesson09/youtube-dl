...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _print_sig_code(self, func, example_sig):
        if False:#not self.get_param('youtube_print_sig_code'):
            return
    ...
    def _extract_n_function(self, video_id, player_url):
        ...
        if False:#self.get_param('youtube_print_sig_code'):
    ...
    def _extract_player_responses(self, clients, video_id, webpage, master_ytcfg):
        ...
        while clients:
            ...
            if True:#'configs' not in self._configuration_arg('player_skip'):
                player_ytcfg = self._extract_player_ytcfg(client, video_id) or player_ytcfg

            player_url = player_url or self._extract_player_url(master_ytcfg, player_ytcfg, webpage=webpage)
            require_js_player = self._get_default_ytcfg(client).get('REQUIRE_JS_PLAYER')
            if False:#'js' in self._configuration_arg('player_skip'):
                require_js_player = False
                player_url = None
    ...
    def _extract_formats(self, streaming_data, video_id, player_url, is_live):
        ...
        for fmt in streaming_formats:
            ...
            query = compat_parse_qs(compat_urllib_parse_urlparse(fmt_url).query)
            throttled = False
            ...
            for lang_code, caption_track in captions.items():
                ...
                for trans_code, trans_name in translation_languages.items():
                    ...
                    if caption_track.get('kind') != 'asr':
                        ...
                        trans_name += "from "+lang_name;#format_field(lang_name, template=' from %s')
                    process_language(
...
class YoutubeTabIE(YoutubeBaseInfoExtractor):
    ...
    def _extract_data(self, url, item_id, ytcfg=None, fatal=True, webpage_fatal=False, default_client='web'):
        data = None
        if True:#'webpage' not in self._configuration_arg('skip'):
...
.*?\?.*?\blist=
    ...
    def _real_extract(self, url):
        ...
        url = update_url_query(
            ...
            compat_parse_qs(compat_urllib_parse_urlparse(url).query) or {'list': playlist_id})
        if is_music_url:
